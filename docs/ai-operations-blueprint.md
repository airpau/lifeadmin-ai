...
### DONE (completed 26 March - evening session)
- ~~Share Your Win~~ (social share modal after complaint/cancel with referral link)
- ~~Credit Score Impact Warning~~ (amber modal before cancelling credit products)
- ~~Price Increase Alerts~~ (daily detection from bank data + email alerts + dashboard banner)
- ~~Smart Bill Comparison~~ (side-by-side alternatives with Awin affiliate links)
- ~~Receipt/Bill Scanner~~ (Claude Vision OCR integrated into AI Letters tab)
- ~~Savings Challenges~~ (12 gamified challenges verified by bank data)
- ~~Annual Financial Report~~ (PDF download, sample preview for non-Pro)
- ~~AI Self-Learning System~~ (confidence-based rules from user corrections, nightly cleanup)
- ~~Universal email scanning~~ (IMAP for any provider + Gmail/Outlook OAuth routes)
- ~~X/Twitter integration~~ (posting + engagement bot + lead capture)
- ~~ElevenLabs TTS~~ (listen to complaint letters, British voice)
- ~~14-day free Pro trial~~ (replaced founding member programme)
- ~~Trial conversion tracking~~ (started/expired/converted timestamps)
- ~~Dual-sided referral reward~~ (Stripe coupon, 1 free month for both parties)
- ~~Churn prevention emails~~ (7d/14d inactive + pre-renewal value summary)
- ~~Meta Ads campaigns~~ (2 campaigns PAUSED with ad sets + images)
- ~~Daily ad metrics cron~~ (Google + Meta, 7am)
- ~~Weekly ad optimisation cron~~ (auto-adjust budgets by CPA)
- ~~Signup attribution table~~ (utm, gclid, fbclid, ref_code)
- ~~4 SEO blog articles~~ (energy, flights, money saving apps, hidden subscriptions)
- ~~Nightly merchant cleanup cron~~ (applies trusted rules to all users)
- ~~AI Letters tab~~ (merged Complaints + Forms, 11 letter types, bill upload)
- ~~Loading animation~~ (rotating captions during letter generation)
- ~~Comparison widget~~ (dashboard shows actual deals with switch links)
- ~~Subscriptions filtered~~ (no mortgages/loans/credit cards, statutory badge)
- ~~Profile completeness indicator~~ (progress bar + nudge)
- ~~Profile connected accounts~~ (shows real bank/email connection status)
- ~~Profile stats from real data~~ (letters generated, money recovered)
- ~~37 UAT bugs fixed~~ (20 v1 + 17 v2 including 3 regressions)
- ~~Homepage comparison table~~ (22 features with all new items)
- ~~Pricing page~~ (updated feature lists, 14-day trial badge on Pro)
- ~~UK journalist PR list~~ (30+ contacts with emails, saved to docs/)
- ~~fal.ai integration~~ (branded image generation for social posts)
- ~~Social media posts~~ (Facebook + Instagram posted with fal.ai images)

### STILL OUTSTANDING
1. Microsoft Azure app credentials (Outlook OAuth - Paul to register)
2. Google Ads Basic API access (waiting for Google approval)
3. Google Ads 3 search campaigns (blocked on #2)
4. Google OAuth verification (Gmail scanning - submitted 24 March)
5. Meta App Review (ad creatives blocked - app in dev mode)
6. Full manual feature testing by Paul
7. Launch preparation (target ~2 April)

### SKIPPED (future versions)
- WhatsApp Bot (cost/complexity not worth it for v1)
- Household Mode (data protection concerns, too complex for v1)
- One-Click Switching (can't pre-fill external forms)
- ElevenLabs video generation (AI video quality not good enough yet)
