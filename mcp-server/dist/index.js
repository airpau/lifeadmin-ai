...
function resolveContextFile(file) {
...
function getMetaToken() {
    ...
    if (!token)
        throw new Error("PAYBACKER_META_TOKEN env var not set");
    return token;
}
...
async function handleTool(name, args) {
    ...
    switch (name) {
        // --- Shared Context ---
        case "read_context": {
            return readContextFile(args.file);
        }
        case "write_context": {
            const path = resolveContextFile(args.file);
        ...
        case "add_task": {
            const priority = args.priority.charAt(0).toUpperCase() + args.priority.slice(1);
        ...
        case "post_to_facebook": {
            ...
            const tokenRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${FACEBOOK_PAGE_ID}?fields=access_token&access_token=${systemToken}`);
            ...
            let postRes;
            if (imageUrl) {
                postRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${FACEBOOK_PAGE_ID}/photos`, {
                    method: "POST",
                ...
                postRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${FACEBOOK_PAGE_ID}/feed`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        message: args.message,
        ...
        case "post_to_instagram": {
            ...
            const containerRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${INSTAGRAM_ID}/media`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            // Step 3: Publish
            const publishRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${INSTAGRAM_ID}/media_publish`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
        ...
        case "get_git_status": {
            try {
                ...
                return `Error reading git status: ${e.message}`;
            }
        }
        case "get_server_health": {
            ...
            try {
                ...
                results.push(`Vercel: DOWN - ${e.message}`);
            }
            ...
            try {
                ...
                results.push(`Railway: DOWN - ${e.message}`);
            }
            return results.join("\n");
        }
        case "read_business_log": {
            ...
            try {
                const res = await fetch(`${url}/rest/v1/business_log?select=*&order=created_at.desc&limit=${limit}`, {
                    headers: {
                        apikey: key,
...
const server = new Server({ name: "paybacker-mcp-server", version: "1.0.0" }, { capabilities: { tools: {} } });
