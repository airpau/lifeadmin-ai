...
function buildEmail(type: ChurnEmailType, name: string, data: Record<string, any>): string {
  const header = `
    <div style="background:#0a1628;padding:24px 32px;border-bottom:1px solid #1e3a5f;text-align:center;">
  ...
  const cta = (text: string, href: string) =>
    `<div style="text-align:center;margin:28px 0;">
      <a href="${href}" style="display:inline-block;background:#34d399;color:#0a1628;font-weight:700;font-size:15px;padding:14px 28px;border-radius:6px;text-decoration:none;">${text}</a>
    </div>`;
  ...
  if (type === 'inactive_7d') {
    ...
    body = `
      ...
      <div style="background:#162544;border-radius:12px;padding:20px 24px;margin:20px 0;border-left:3px solid #34d399;">
        ...
        ${expiringCount > 0 ? `<p style="color:#f59e0b;margin:0 0 8px;font-size:14px;font-weight:600;">${expiringCount} contract${expiringCount > 1 ? 's' : ''} expiring soon. Review before they auto-renew at a higher rate.</p>` : ''}
  ...
  if (type === 'inactive_14d') {
    body = `
      ...
      <div style="background:#162544;border-left:3px solid #34d399;border-radius:0 8px 8px 0;padding:16px 20px;margin:20px 0;">
  ...
  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:600px;margin:0 auto;background:#0a1628;border-radius:16px;overflow:hidden;">
      ${header}
