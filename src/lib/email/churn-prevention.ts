...
function buildEmail(type: ChurnEmailType, name: string, data: Record<string, any>): string {
  ...
  const cta = (text: string, href: string) =>
    `<div style="text-align:center;margin:28px 0;">
      <a href="${href}" style="display:inline-block;background:#f59e0b;color:#0f172a;font-weight:700;font-size:15px;padding:14px 28px;border-radius:12px;text-decoration:none;">${text}</a>
    </div>`;
  ...
  if (type === 'inactive_7d') {
    ...
    body = `
      ...
      <div style="background:#162544;border-radius:12px;padding:20px 24px;margin:20px 0;border-left:3px solid #f59e0b;">
        <p style="color:#f59e0b;font-weight:700;margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;">Your snapshot</p>
  ...
  if (type === 'pre_renewal') {
    ...
    body = `
      ...
      ${lettersGenerated > 0 ?
        `<p style="color:#f59e0b;font-size:15px;text-align:center;margin:0 0 16px;font-weight:600;">You are actively taking charge of your finances.</p>` :
