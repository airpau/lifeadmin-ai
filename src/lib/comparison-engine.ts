...
interface DealData {
  ...
  awinMid: string;
  providerUrl: string;
  /** Prebuilt tracking URL if available (from affiliate_deals.affiliate_url) */
  affiliateUrl?: string;
}

/**
 * Map deals-engine category keys to the category values stored in affiliate_deals.
 * The DB currently stores: broadband, energy, mobile.
 * Keep variants here so future seeds (mortgages, loans, credit_card etc.) work
 * regardless of singular/plural naming.
 */
const DEAL_CATEGORY_DB_VARIANTS: Record<string, string[]> = {
  energy: ['energy'],
  broadband: ['broadband'],
  mobile: ['mobile'],
  insurance: ['insurance'],
  ...
  loans: ['loan', 'loans'],
  'credit-cards': ['credit_card', 'credit-cards', 'credit cards'],
  'car-finance': ['car-finance', 'car_finance'],
  travel: ['travel'],
  water: ['water'],
};

/**
 * Load deals for a category from affiliate_deals (source of truth for the deals page).
 * Falls back to an empty list if the category has no DB rows yet — callers can merge
 * hardcoded fallbacks as needed.
 */
async function fetchDealsFromDb(dealCategory: string): Promise<DealData[]> {
  const variants = DEAL_CATEGORY_DB_VARIANTS[dealCategory] || [dealCategory];
  const admin = getAdmin();
  const { data, error } = await admin
    .from('affiliate_deals')
    .select('id, provider, plan_name, price_monthly, price_promotional, awin_advertiser_id, destination_url, affiliate_url, category')
  ...
  return data.map((row: any) => {
    const headlinePrice = row.price_promotional != null && row.price_promotional > 0
      ? parseFloat(String(row.price_promotional))
      : parseFloat(String(row.price_monthly || 0));
    return {
      id: row.id,
      provider: row.provider,
      headline: row.plan_name || row.provider,
      monthlyPrice: headlinePrice,
      awinMid: row.awin_advertiser_id ? String(row.awin_advertiser_id) : '',
      providerUrl: row.destination_url || '',
...
/**
 * Return the best tracking URL we have for a deal — prefer the stored affiliate_url
 * (which was generated by the Awin Link Builder) and fall back to a constructed
 * URL if we only have the raw awinMid + providerUrl (e.g. hardcoded fallback rows).
 */
function dealTrackingUrl(d: DealData): string {
  if (d.affiliateUrl) return d.affiliateUrl;
  if (d.awinMid && d.providerUrl) return buildAwinUrl(d.awinMid, d.providerUrl);
...
/**
 * Fallback deals for categories that are not yet seeded into affiliate_deals.
 * The deals page (and `fetchDealsFromDb`) is the primary source of truth; these
 * rows are only merged in when a category returns zero DB rows.
 */
const FALLBACK_DEALS_BY_CATEGORY: Record<string, DealData[]> = {
  energy: [
...
function normaliseToDealCategory(sub: {
  ...
  if (sub.category_normalized && DEAL_CATEGORY_DB_VARIANTS[sub.category_normalized]) {
...
export async function findCheaperAlternatives(
  ...
  // Primary source of truth: affiliate_deals table (same catalog as the deals page).
  // Fallback to hardcoded list only when a category has no DB rows yet (e.g. insurance, mortgages).
  const dbDeals = await fetchDealsFromDb(dealCategory);
  const deals: DealData[] = dbDeals.length > 0 ? dbDeals : (FALLBACK_DEALS_BY_CATEGORY[dealCategory] || []);
