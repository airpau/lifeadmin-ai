  ...
  for (const msg of messages) {
      ...
      if (codeMatch) {
        ...
        if (codeRow) {
          ...
          if (sessErr) {
            ...
            // Fire the Meta-approved welcome template too, so we have a
            // formal opt-in record + the user sees the same wording any
            // future audit might inspect. Best-effort: any failure is
            // logged but doesn't break the link flow.
            try {
              ...
              console.warn('[whatsapp/webhook] welcome template send failed', e);
            }
          }
          processed += 1;
          continue;
    ...
    if (msg.text && /^\s*stop\s*$/i.test(msg.text)) {
      ...
      try {
        ...
        await sendWhatsAppTemplate({
          to: msg.from,
          templateName: 'paybacker_opted_out',
          parameters: [],
        });
      } catch (e) {
        // Fall back to in-window text if the template send 4xxs
        // (e.g. window race or upstream Twilio blip).
        console.warn('[whatsapp/webhook] opted_out template send failed', e);
