...
export async function POST(req: NextRequest) {
  ...
  if (supabase) {
    try {
      ...
      if (!error) {
        ...
        // Postgres unique-violation — they're already on file.
        duplicated = true;
        insertOk = true;
      } else {
        // Table missing (42P01) or schema mismatch — log and let Resend
        // act as the fallback delivery channel.
  ...
  // 2) Resend notification — awaited so we can report total failure.
  let emailOk = false;
  if (process.env.RESEND_API_KEY) {
    const subject = duplicated
      ? `Careers interest (returning) · ${fullName}`
    ...
    try {
      ...
      console.error('[careers] resend notify failed', err);
    }
  }

  // If neither channel delivered, surface a real error so the user
  // knows to try again rather than assuming success.
  if (!insertOk && !emailOk) {
    return NextResponse.json(
      {
        ok: false,
        error:
          'We could not record your interest right now. Please try again or email hello@paybacker.co.uk directly.',
      },
      { status: 500 },
    );
  }
