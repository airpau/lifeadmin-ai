 ...
 *   "Client Payment"). When set, the transaction is stamped with
 *   user_category='income' and user_subcategory=<label> so income/spending
 *   analysis keeps grouping these as income while the label surfaces in the
 *   drill-down. Parent context ('income') comes from the income sheet.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (newIncomeType) {
      ...
      // 'income' so income/spending analysis keeps grouping it correctly and
      // the user_category_custom registry's usage-count join in
