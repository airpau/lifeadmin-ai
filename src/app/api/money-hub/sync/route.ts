...
export async function POST() {
  ...
  // Step 3a: Identify business accounts. is_business column may be unset
  // in older rows — fall back to the bank-name heuristic so HSBC Business,
  // Tide, Mettle etc. still get the smarter classifier without a migration
  // backfill.
  const { data: connRows } = await admin.from('bank_connections')
    .select('id, bank_name, is_business')
    .eq('user_id', user.id);
  const businessConnIds = new Set<string>();
  for (const c of connRows ?? []) {
    const flag = (c as { is_business?: boolean }).is_business === true;
    if (flag || looksLikeBusinessBankName(c.bank_name)) {
      businessConnIds.add(c.id);
    }
  }
  ...
  for (const txn of transactions) {
    ...
    if (amount > 0) {
      ...
      const isBusinessAccount = businessConnIds.has(
