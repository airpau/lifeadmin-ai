...
- [ ] Frontend: Move Savings Goals above Financial Actions Centre - In money-hub page.tsx, move the Savings Goals section to sit next to the Budget Planner, above the Financial Actions Centre. Paul says it's currently too hard to find. (@Claude Code)
...
- [ ] Phase 2 Gap: Add disclaimer footer to all AI-generated letters - Every AI letter needs: "This letter was generated by AI using publicly available legal information. It does not constitute legal advice. If unsure, consult a qualified solicitor or Citizens Advice." Add to letter generation output AND any PDF/email exports. (@Claude Code)
...
- [ ] Build iOS + Android app (Expo/React Native) - Create mobile apps for App Store and Google Play. Recommended approach: Expo (React Native) with WebView wrapper for initial release, then gradually convert key screens to native components. Requires: app icons, splash screens, push notification setup, deep linking. Must handle TrueLayer Open Banking auth flow and Supabase auth on mobile. Blocked by: Google OAuth verification and TrueLayer production approval must be cleared first. (@Claude Code + Cowork architecture)
...
- [ ] FCA COMPLIANCE: Replace Money Hub Net Position card with Savings Rate - The Money Hub "Net position" card (3rd card in top row) shows a £ value (-£6,154.75) which acts as a bank balance proxy. Under PSD2/PSRs, displaying consolidated account information (including balances) requires FCA agent registration which we do not yet have. Replace with a "Savings Rate" card showing a percentage: calculate as (income - spending) / income * 100. Show green if positive, red if negative. Also create lib/feature-flags.ts with SHOW_BANK_BALANCES: false to gate any future balance display. See claude-code-moneyhub-fca-compliance.md for full specification. (@Antigravity)
- [ ] Frontend: Replace localStorage dismiss with Supabase RPCs - In src/app/dashboard/money-hub/page.tsx: Replace dismissBill/undismissBill/clearAllDismissals functions to use supabase.rpc('dismiss_expected_bill'), supabase.rpc('restore_expected_bill'). Remove localStorage key pb_dismissed_bills_YYYY_MM. Load dismissed bills from dismissed_expected_bills table on mount. See /mnt/outputs/money-hub-fixes.md for exact code patches. (@Claude Code)
...
- [ ] Frontend: Use get_monthly_spending RPC for spending totals - Replace frontend spending total calculation with supabase.rpc('get_monthly_spending_total', {p_user_id, p_year, p_month}). Use get_monthly_spending() for category breakdown. These exclude transfers and income automatically. Feb 2026 correctly shows £20,618.55 instead of £76,723. (@Claude Code)
- [ ] Bank-sync: Call fix_ee_card_merchant_names and detect_and_sync after sync - In the bank-sync edge function, after upserting transactions: 1) Call fix_ee_card_merchant_names(user_id) to fix EE-branded card merchants. 2) Call detect_and_sync_recurring_transactions(user_id) to flag recurring payments and auto-create subscriptions. 3) Call auto_categorise_transactions(user_id) for automatic categorisation. These must run for EVERY user on every sync, not just Paul's account. (@Claude Code)
