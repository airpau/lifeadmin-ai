...
- [x] Implement global email rate limiter (max 2 emails per user per day) — rate limiter live in `src/lib/email-rate-limit.ts` and now includes `contract_expiry_alert`, `contract_end_alert`, `overcharge_alert` (lines 36-38). PR#99 was closed without merge but the alert types landed via a later commit. Verified 2026-05-12.
...
- [x] Frontend: Move Savings Goals above Financial Actions Centre — `GoalsAndBudgetsPanel` renders at money-hub/page.tsx:1170, FAC at line 1336. PR#78 merged 7a07d564. Verified 2026-05-12.
...
- [x] Frontend: Add spending category totals to breakdown — grand total now uses authoritative server-computed `spending.totalSpent` so the footer matches the banner across all plan tiers. PR#304 merged ae18d64d. Verified 2026-05-12.
...
- [x] Money Hub: Mobile layout fixes — net worth overlap, regular payments overflow, hidden action buttons — fix landed in commit fcb71624 (5 May 2026): NetWorthPanel drops `h-full`, adds `min-w-0`/truncate; PaymentCard delete button is `opacity-100` below the sm breakpoint. PR#43 itself was only the in-progress marker. Verified 2026-05-12.
...
- [x] Frontend: Replace localStorage dismiss with Supabase RPCs — money-hub/page.tsx:279 now calls `supabase.rpc('dismiss_expected_bill', ...)`, with matching restore RPC; localStorage `pb_dismissed_bills_*` key removed. PR#49 merged 0f9b00fa. Verified 2026-05-12.
...
- [x] Bank-sync: Call fix_ee_card_merchant_names and detect_and_sync after sync — all three RPCs now run after sync for every user. Wired into `/api/cron/bank-sync` (route.ts:383-386), `/api/bank/sync-now` (route.ts:376-378), and `/api/yapily/initial-sync` (route.ts:173-175). PR#192 merged cb51cc7e. Verified 2026-05-12.
...
- [x] Frontend: Use get_subscriptions_with_actions RPC for subscription list — N+1 elimination merged in PR#389 (d4c2752b). `/api/subscriptions` now batch-attaches `cancellation_info` + `has_cancellation_info` inline from `provider_cancellation_info`; subscriptions/page.tsx:279-285 seeds from the inline payload. Verified 2026-05-12. Follow-up split out below.
- [ ] Frontend follow-up to PR#389: consistent "Generate Cancellation Email" / "Mark as Cancelled" action buttons on every subscription row + wire `generate_cancellation_email` RPC for the email body. (@Claude Code)
- [x] Frontend: Use dismiss_subscription/cancel_subscription RPCs and update total — DELETE/PATCH on `/api/subscriptions/[id]` now return updated totals from `dismiss_subscription`/`cancel_subscription`; subscriptions/page.tsx:823 and :1010 apply them immediately without a separate fetch. PR#365 merged 62e2fd18. Verified 2026-05-12.
...
- [x] Apply frontend fixes from paybacker-frontend-fixes.md — umbrella task now superseded by individual PRs. C1-C4 spending/income RPCs: PR#72 (still open). C5 localStorage→RPC dismiss: PR#49 merged. C6 dismiss/cancel RPC totals: PR#365 merged. H1-H2 subscription count + annual total: PR#389 merged (N+1 elimination). H3 consistent action buttons: tracked as PR#389 follow-up above. H4 income breakdown RPC: PR#72 (open). M4 Savings Goals position: PR#78 merged. The reference doc `paybacker-frontend-fixes.md` no longer exists in the repo. Closing umbrella; remaining work tracked by the open PRs above. Verified 2026-05-12.
