...
- [x] Frontend: Move Savings Goals above Financial Actions Centre (Merged PR#78 — Savings Goals now sits next to Budget Planner above the Financial Actions Centre in Money Hub.)
...
- [x] Frontend: Add spending category totals to breakdown (Merged PR#304 — Money Hub spending grand total now matches the banner for all plan tiers.)
- [x] Fix URL routing: /dashboard/disputes and /dashboard/overview return 404 (Verified resolved on master 2026-05-29 — src/app/dashboard/disputes/page.tsx and src/app/dashboard/overview/page.tsx both exist and resolve, no longer 404. PR#482 also flagged this as complete.)
- [x] Money Hub: Mobile layout fixes — net worth overlap, regular payments overflow, hidden action buttons (Merged PR#43 — action buttons now visible on touch devices and responsive layout corrected.)
...
- [x] Frontend: Replace localStorage dismiss with Supabase RPCs (Merged PR#49 — Money Hub dismiss flow now uses dismiss_expected_bill / restore_expected_bill RPCs against dismissed_expected_bills table.)
...
- [x] Bank-sync: Call fix_ee_card_merchant_names and detect_and_sync after sync (Merged PR#192 — Yapily initial-sync now invokes fix_ee_card_merchant_names + detect_and_sync_recurring_transactions for every user.)
...
- [x] Frontend: Use get_subscriptions_with_actions RPC for subscription list (Merged PR#389 inlined cancellation_info + has_cancellation_info on /api/subscriptions, eliminating N+1 fetches. Consistent action buttons ("Generate Cancellation Email" + "Mark as Cancelled") were already rendered for every non-statutory active subscription in src/app/dashboard/subscriptions/page.tsx lines 2583-2602 as of 2026-05-29.)
- [x] Frontend: Use dismiss_subscription/cancel_subscription RPCs and update total (Merged PR#365 — frontend now applies the RPC-returned monthly_total/annual_total/active_count immediately after dismiss/cancel, no separate fetch needed.)
